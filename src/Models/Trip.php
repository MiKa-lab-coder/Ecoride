...
class Trip extends BaseModel
{
    ...
    public function getDepartureDay(): DateTime { return $this->departure_day; }
    public function getArrivalDay(): DateTime { return $this->arrival_day; }
    public function getDepartureLocation(): string { return $this->departure_location; }
    public function getArrivalLocation(): string { return $this->arrival_location; }
    ...
    public function getTripTime(): int { return $this->trip_time; }
    public function getTripPrice(): int { return $this->trip_price; }
    public function getTripNature(): string { return $this->trip_nature; }
    public function getAnimalPref(): bool { return $this->animal_pref; }
    public function getSmokingPref(): bool { return $this->smoking_pref; }
    public function getSeating(): int { return $this->seating; }
    ...
    public function getDriverFirstname(): ?string { return $this->driver_firstname; }
    public function getDriverName(): ?string { return $this->driver_name; }
    ...
    public function setDepartureDay(DateTime $departure_day): void { $this->departure_day = $departure_day; }
    public function setArrivalDay(DateTime $arrival_day): void { $this->arrival_day = $arrival_day; }
    public function setDepartureLocation(string $departure_location): void { $this->departure_location = $departure_location; }
    public function setArrivalLocation(string $arrival_location): void { $this->arrival_location = $arrival_location; }
    public function setDepartureTime(DateTime $departure_time): void { $this->departure_time = $departure_time; }
    public function setArrivalTime(DateTime $arrival_time): void { $this->arrival_time = $arrival_time; }
    public function setTripTime(int $trip_time): void { $this->trip_time = $trip_time; }
    public function setTripPrice(int $trip_price): void { $this->trip_price = $trip_price; }
    public function setTripNature(string $trip_nature): void { $this->trip_nature = $trip_nature; }
    public function setAnimalPref(bool $animal_pref): void { $this->animal_pref = $animal_pref; }
    public function setSmokingPref(bool $smoking_pref): void { $this->smoking_pref = $smoking_pref; }
    public function setSeating(int $seating): void { $this->seating = $seating; }
    public function setStatus(string $status): void { $this->status = $status; }
    public function setDriverId(int $driver_id): void { $this->driver_id = $driver_id; }
    public function setVehicleId(int $vehicle_id): void { $this->vehicle_id = $vehicle_id; }

    // Setters pour les nouvelles propriétés
    public function setDriverFirstname(?string $driver_firstname): void { $this->driver_firstname = $driver_firstname; }
    public function setDriverName(?string $driver_name): void { $this->driver_name = $driver_name; }
    public function setBrand(?string $brand): void { $this->brand = $brand; }
    public function setModel(?string $model): void { $this->model = $model; }
    public function setRegistrationNumber(?string $registration_number): void { $this->registration_number = $registration_number; }
        ...
        try {
            if ($this->getTripId() !== null) {
                $stmt = $db->prepare("UPDATE TRIPS SET departure_day = :departure_day, arrival_day = :arrival_day,
                 ...
                 departure_time = :departure_time, arrival_time = :arrival_time, trip_time = :trip_time,
                 trip_price = :trip_price, trip_nature = :trip_nature, animal_pref = :animal_pref,
                 smoking_pref = :smoking_pref, seating = :seating, status = :status, driver_id = :driver_id,
                 vehicle_id = :vehicle_id WHERE trip_id = :trip_id");
                $stmt->bindValue(':trip_id', $this->getTripId(), PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare("INSERT INTO TRIPS (departure_day, arrival_day, departure_location,
                   arrival_location, departure_time, arrival_time, trip_time, trip_price, trip_nature, animal_pref,
                   smoking_pref, seating, status, driver_id, vehicle_id)
                VALUES (:departure_day, :arrival_day,:departure_location, :arrival_location, :departure_time,
                        :arrival_time, :trip_time, :trip_price, :trip_nature, :animal_pref, :smoking_pref,
    ...
    public static function getTripsByDriverId(int $driver_id): array
    {
        ...
        try {
            $stmt = $db->prepare("
                ...
                WHERE t.driver_id = :driver_id AND t.status = 'approved' 
    ...
    public static function getCompletedTripsByUser(int $userId): array
    {
        ...
        try {
            $stmt = $db->prepare("
                SELECT DISTINCT t.*, u.name as driver_name, u.firstname as driver_firstname
                FROM TRIPS t
            ...
            $trips = [];
            foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $tripData) {
                $isDriver = ($userId === (int)$tripData['driver_id']);
                
                if ($isDriver) {
                    $passengers = Booking::getPassengersForTrip($tripData['trip_id']);
                    foreach ($passengers as $passenger) {
                        $tripData['user_to_rate_id'] = $passenger['user_id'];
                        $tripData['user_to_rate_name'] = $passenger['firstname'] . ' ' . $passenger['name'];
                        $tripData['has_rated'] = Rating::hasUserRatedTrip($userId, $tripData['trip_id'], $passenger['user_id']);
                        $trips[] = $tripData;
                    }
                } else {
                    $tripData['user_to_rate_id'] = $tripData['driver_id'];
                    $tripData['user_to_rate_name'] = $tripData['driver_firstname'] . ' ' . $tripData['driver_name'];
                    $tripData['has_rated'] = Rating::hasUserRatedTrip($userId, $tripData['trip_id'], $tripData['driver_id']);
                    $trips[] = $tripData;
                }
            }
            return $trips;
    ...
    public function toArray(): array
    {
        return [
            ...
            'driver_firstname' => $this->driver_firstname ?? null,
